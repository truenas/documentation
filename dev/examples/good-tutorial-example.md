<!-- 
EXEMPLARY TUTORIAL DOCUMENTATION: Adding Cloud Credentials
Source: /content/SCALE/SCALETutorials/Credentials/BackupCredentials/AddCloudCredentials.md

WHY THIS IS AN EXCELLENT TUTORIAL EXAMPLE:
1. **Perfect H1-H2-H3 Hierarchy**: Uses clear heading structure throughout
2. **Task-Oriented Content**: Focuses on "how to do X" with clear numbered steps
3. **Strong Prerequisites**: Sets expectations upfront about what users need
4. **Multiple Scenarios**: Covers various cloud providers systematically
5. **Proper Cross-References**: Links to related UI reference documentation
6. **Good Screenshot Usage**: Uses {{< trueimage >}} shortcode properly
7. **Consistent Voice**: Uses imperative ("Click", "Enter", "Select") throughout
8. **Include File Usage**: Proper use of {{< include file >}} for reusable content
9. **Clear Sections**: Each provider gets its own well-structured section
10. **Security Awareness**: Includes important security warnings and notes

STYLE GUIDE PATTERNS DEMONSTRATED:
- Tutorial heading hierarchy (H1 main topic, H2 procedures, H3 sub-steps)
...
1. Select the cloud service from the **Provider** dropdown list. The provider required authentication option settings display.
   ...
   For details on each provider authentication settings see [Cloud Credentials Screens]({{< ref "/SCALE/SCALEUIReference/Credentials/BackupCredentials/CloudCredentialScreens" >}}).
