...
2. Reload the **Datasets** page in the WebUI and confirm no datasets in Pool B show sharing icons. Open the **Sharing** page and confirm all share paths now point to Pool A.
   ...
   Do not delete and re-add SMB shares. Doing so removes the existing share ACL and replaces it with an open ACL, even if you maintain the exact share settings in the UI.
   {{< /hint >}}
   ...
   After you change and save the path, view the SMB share ACL and verify it is identical to the ACL you noted before you changed the configuration.
   {{< /expand >}}
   ...
   {{< expand "iSCSI / NVMeT / FC Migration" "v" >}}
...
4. Reload the **Datasets** page and confirm no stray backup tasks remain on Pool B. Open the **Data Protection** page and confirm no data protection tasks are active for Pool B.
   ...
   Depending on the TrueCloud service configuration (for example, if you use absolute paths), remote storage consumption can increase significantly after a path change. If you use absolute paths, you might need to monitor space usage and clean up old backups from the previous pool.
   {{< /hint >}}
   ...
   {{< expand "Cloud Sync Tasks Migration" "v" >}}
   #### Cloud Sync Tasks Migration
