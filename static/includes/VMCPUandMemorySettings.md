...
| **Optional: CPU Set (Examples: 0-3,8-11)** | Specify the logical cores that the VM is allowed to use. Better cache locality can be achieved by setting the CPU based on CPU topology. E.g. to assign cores: 0,1,2,5,9,10,11 you can write: `1-2,5,9-11` |
...
| **Optional: NUMA nodeset (Example: 0-1)** | Node set allows setting NUMA nodes for multi NUMA processors when CPU set was defined. To achieve better memory locality, setting nodeset based on assigned cpuset. For example, if cpus 0,1 belong to NUMA node 0, setting nodeset to 0 improves memory locality. |
{{< /truetable >}}
{{< /expand >}}
