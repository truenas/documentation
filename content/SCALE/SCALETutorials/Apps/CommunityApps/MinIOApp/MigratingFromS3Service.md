---
title: "Migrating from S3 service to MinIO app"
description: "Provides migration instructions on how to move from the deprecated TrueNAS built-in S3 service to the MinIO server application."
weight: 10
aliases:
tags:
- scaleminio
...
MinIO no longer supports these offerings, does not provide a direct upgrade path for either, and requires users to migrate from these S3 services to a later MinIO Server release.

TrueNAS SCALE offers access to the current MinIO release through the SCALE MinIO application.
...
{{< enterprise >}}
Enterprise customers are strongly encouraged to contact iXsystems Support for assistance with the migration process.
{{< /enterprise >}}
...
The migration must be completed on TrueNAS SCALE Bluefin (latest maintenance release is recommended).
Later TrueNAS SCALE major versions do not have the built-in S3 service and thus cannot migrate.
...
Completing the migration allows upgrading TrueNAS SCALE from Bluefin to Cobia (or later major versions).
...
1. Create a new [dataset](https://www.truenas.com/docs/scale/22.12/scaletutorials/storage/datasets/datasetsscale/) named **data** for the MinIO app.
   ...
   Make sure this allocated data storage does not cause the pool to exceed 80% utilization during the migration process.
...
4. Log into the MinIO web portal to migrate the S3 service configuration settings and bucket(s) with bucket settings.
   ...
   4. Create a new bucket for the MinIO app. When the S3 service deployment has multiple buckets, create the same number for the MinIO app deployment.
      The lifecycle rules, versioning, quotas, and locks for the new buckets must match those applied to the S3 deployment buckets.
      ...
      {{< /expand >}}
      After manually recreating all settings and buckets, migrate bucket contents with the `mc mirror` command (see step 8).

5. Install the TrueNAS MinIO app.
   {{< enterprise >}}
   Enterprise users should contact iXsystems Support for assistance with the MinIO **Enterprise** app.
   ...
   6. Click **Save** to complete the TrueNAS app installation and deployment.
