...
You can allow SCALE to create these storage volummes or you can create and name datasets according to your intented use or as sequentially-named datasets (i.e., *volume1*, *volume2*, etc.) for each extra volume to mount inside the container.
...
To make sure your plots and wallet private key persists (is not lost) across container restarts, save the mnemonic seed created during the app installation and deployment. To do this:
...
To show Chia key file details and the 24 word recovery key, enter `/chia-blockchain/venv/bin/chia keys show --show-mnemonic-seed` at the **#** prompt. 
...
To copy from the SCALE **Pod Shell**, highlight the part of the screen you want to copy, then press <kbd>Ctrl Insert</kbd>. 
Open a text editor file like Notepad, paste the information into the file and save it. Back up this file and keep it secured where only authorized people can access it.
