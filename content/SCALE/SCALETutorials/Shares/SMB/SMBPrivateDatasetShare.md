...
While you can add a share while [creating the dataset]({{< relref "DatasetsSCALE" >}}) on the **Add Dataset** screen, this method requires editing the share settings afterward.
In general, when creating a simple SMB share and dataset you can use either method, but if customizing the dataset, use the [**Add Dataset** screen]({{< relref "DatasetsSCALE" >}}) to access dataset advanced setting options.
...
See [Setting Up Permissions]({{< relref "PermissionsSCALE" >}}) for more information on editing dataset permissions.
...
See [SMB Shares]({{< relref "ManageSMBShares" >}}) for detailed information on editing the share dataset permissions.
...
Go to **Storage** and [create a pool]({{< relref "CreatePoolWizard" >}}).

Next, [set up the Active Directory]({{< relref "/SCALE/SCALETutorials/credentials/directoryservices/configadscale" >}}) that you want to share resources with over your network.
