...
See [Adding SSH Credentials]({{< relref "AddSSHConnectionKeyPair" >}}) for more information.
...
The [Rsync Daemon]({{< relref "Rsyncd" >}}) application is available in situations where configuring TrueNAS as an rsync server with an rsync module is necessary.
...
2. Create an rsync module on the remote system.
   On TrueNAS, install an rsync app (such as [Rsyncd]({{< relref "Rsyncd" >}})) and configure the module.
