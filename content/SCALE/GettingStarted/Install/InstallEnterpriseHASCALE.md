...
{{< hint danger >}}
Installing a SCALE Enterprise (HA) dual controller system is a complicated process. We recommend Enterprise customers contact iXsystems Support for assistance with installing or recovering their system using the clean install process!
Customers that proceed independently should NOT use Linux or CLI commands to recover or clean-install the SCALE <kbd>iso</kbd> file or configure any initial configuration settings!
...
{{< hint warning >}}
This article is a work in progress and subject to change as the SCALE release changes with new features and functions.
{{< /hint >}}
...
* SCALE license file
* SCALE controller A and B serial numbers (refer to contracts or documentation provided with the system, or contact iXsystems Support and provide your contract number)
{{< hint info >}}
...
{{< /hint >}}
If restoring after a clean install, also have ready:
* Storage data backup file to import data into the Enterprise HA system
* System configuration file and saved
...
4. Use the DHCP-assigned IP address or assign the controller A IP address using the [Console setup menu](#configuring-the-network-with-console-setup-menu) to gain access to the SCALE UI.
   ...
   If you are experienced with the Console setup menu and using it to configure network settings you can configure the rest of the controller A network settings with the Console setup menu, but we recommend you us the SCALE UI as it has safety mechanisms in place to prevent disrupting network access that could require you to repeat the clean install to access your system.

5. In a separate web browser session log into IPMI using the network address assigned for controller B. 
   Leave controller A IPMI connection up and where you left it at after completing step 4, and then [repeat step 3 for controller B](#using-ipmi-to-install-the-iso-on-a-controller).
...
9. Sign in using the virtual IP address.
...
If installing through an IPMI connection you do not need to save the <kbd>.iso</kbd> file to a USB flash drive. 
If installing at the TrueNAS SCALE server, save the <kbd>.iso</kbd> file to a USB flash drive and use that as the install media.
...
1. Enter the IP address assigned to controller A into a web browser and log into your IPMI system with admin credentials.

2. Select **Remote Control > iKVM/HTML5** to open the Console Setup window. 

   IPMI systems can vary but they use some version of the **Remote Control** and **iKVM/HTML5** option to open a console session.
...
3. Install the <kbd>.iso</kbd> file. Select the **Virtual Media > CD-ROM image** option in your IPMI system.
   ...
   b. Enter the path to the <kbd>.iso</kbd> file. 
      For example, if you stored the file in the *iso* folder enter **/iso/TrueNAS-SCALE-22.12.1.iso** in **Path to Image**.
   c. Click **Save**, then **Mount**. You should see the <kbd>.iso</kbd> file under **Device 1** or the device name your IMPMI configures.
...
3. Return to the **Remote Control > iKVM/HTML5** window opened in step 2. Either use your keyboard or open the keyboard in the window then:
   ...
   c. Select **UEFI: ATEN Virtual CDROM YSOJ** from the boot list. The bootstrap loader begins. When it ends the SCALE installer opens.
{{< /expand >}}
...
{{< hint warning >}}
Only users with experience configuring network setting and with using Console setup menu should use it to configure all network settings. All other users should only use it to configure a static IP address for the primary network interface for controller a to allow access to the SCALE UI. 
...
* VIP to provide UI access regardless of which controller is active. 
  If your system fails over from controller A to B, then fails over back to a later you might not know which controller is active. 
* IP for controller A. If enabled on your network, DHCP assigns only the controller A IP address. If not enabled, you must change this to the static IP address your network admin assigned to this controller.
...
SCALE safeguards allow you 60 seconds to test and save changes to a network interface before reverting changes. This is to prevent users from breaking their network connection in SCALE.
...
3. Edit the primary network interface.
   ...
   After editing the interface settings the **Test Changes** button displays. You have 60 seconds to test then save changes before they revert. If this occurs, edit the interface again.
