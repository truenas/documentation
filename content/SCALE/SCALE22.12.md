...
|---------|-----|---------|------------|-------------|
| 22.12.0 | N/A | App deployment can get stuck in validation when the Host Path is shared between Apps and SMB or NFS sharing services. | Shared host paths are considered insecure and are not recommended. Review host paths used by Apps and Sharing services and adjust paths to be unique. As a last resort that can result in system and app instability, **Host Path Safety Checks** can be disabled in **Apps > Settings > Advanced Settings**. | N/A |
