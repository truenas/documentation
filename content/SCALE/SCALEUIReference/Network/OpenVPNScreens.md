...
The **OpenVPN Client** screen displays the settings for configuring or managing an OpenVPN client. Many settings on this screen are the same as those on the **OpenVPN Server** screen.
...
| **Device Type** | Select a virtual network interface from the dropdown list of options **TUN** or **TAP**. Select the same device type for client and server settings, for example, both must be set to **TUN** or **TAP**. <br> More information is found [here](https://community.openvpn.net/openvpn/wiki/BridgingAndRouting). |
