...
### Choose a Pool for Apps {id="apps_choose-pool"}

**Choose Pool**  opens the **Choose a pool for apps** dialog.
**Pool** sets the storage pool for apps from the dropdown list of available pools on the system.
...
**Migrate existing applications** shows when you select a new pool in the **Pool** dropdown after previously configuring the apps service.
...
#### Delete Image {id="apps_delete_image"}
...
#### Create Docker Registry {id="apps_docker_reg"}
...
| **URI** | The valid Uniform Resource Identifier (URI) for the registry, for example *https://index.docker.io/v1/*. Shows when **URI** is set to **Other Registry** or when a Docker Hub registry record is already configured. |
...
**Preferred Trains** shows a checklist of appliation train options.
Selecting the checkbox to the left of the train name adds that train to the applications catalog.
You must specify at least one train. Train options:
...
* **dev** for developer users, shows apps not yet available to the community
* **test** for applications in development but not yet released in one of the other three trains (stable, enterprise, and community).

**Address Pools** shows the current IP address and subnet mask for the network used by applications.
Select a predefined range from the dropdown list. 

**Add** shows the block of base settings.

**Base** shows the default IP address and subnet.
...
**Registry Mirrors** shows settings to add a registry mirror.
A *registry mirror* is an alternative server used to cache and serve container images, helping speed up downloads and potentially reduce network load. It can be public or private. Mark as insecure for HTTP or HTTPS with a self-signed certificate.
These are alternative Docker/OCI registry endpoints that TrueNAS uses instead of (or in addition to) the default Docker Hub when pulling container images. Useful for air-gapped environments, corporate proxies, or to avoid Docker Hub rate limits.
mirr
**Add** shows a block of registry mirror settings, with:

* **Miror URL** adds the registry mirror URL to TrueNAS that is use when pulling container images.
  It must be a valid URL (e.g., https://mirror.example.com).
* **Insecure** allows the registry mirror to use an unverified or self-signed TLS certificate.
  Enable only for trusted internal mirrors. Not recommended for production use.
...
Select an application row to view the information cards for that application.
Information in the cards changes based on the app row selected in the **Applications** table.

### Application Info Card

The **Application Info** card shows the name, (upstream) app version number, catalog version number, source link for the application, and train name.
...
**Update** shows on the **Application Info** card after clicking **Update All** on the **Installed** applications header.
Both only show if TrueNAS detects an available update for an application.
The application card on the **Discover** screen also displays an update badge.
...
The **Notes** card shows information about the apps, TrueNAS Documentation Hub article locations, links to file bug reports through Jira or GitHub, and where to make feature requests.
...
**Filters** shows a list of sort categories that alter which application cards show. Click on a category to select and filter app cards.
...
## Application Information Screens {id=""}
...
The application information screen shows two cards:
...
Some applications might also include the **Run-As Content** and **Capabilities** cards.
