...
| **Domain Account Name** | (Required) Enter the Active Directory administrator account name. This is the user or service account name used to log into your Active Directory domain controller and that manages access to the resources. Not editable after saving. |
| **Domain Account Password** | (Required) Enter the password for the Active Directory administrator account. Required the first time you configure a domain. After initial configuration, the password is not needed to edit, start, or stop the service. After the initial configuration or joining, TrueNAS uses the **Kerberos Principal** instead of the password. |
