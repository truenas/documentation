...
| Setting | Description |
|---------|-------------|
| **No presets** | Select  to retain control over all **Advanced Options** settings. Utilizing this option gives users the flexibility to manually configure SMB parameters. |
...
| **Multi-User time machine** | Select to set up a multi-user time machine share. This option allows multiple users to utilize TrueNAS as a centralized backup solution while simultaneously ensuring that each user's backups are kept separate and secure from one another.  |
...
| **SMB WORM. Files become read-only via SMB after 5 minutes** | The **SMB WORM** preset only impacts writes over the SMB protocol. Before deploying this option in a production environment, the user should determine whether the feature meets their requirements. By employing this option, users can ensure that data written to the share cannot be modified or deleted, thus increasing overall data integrity and security. |
{{< /truetable >}}
