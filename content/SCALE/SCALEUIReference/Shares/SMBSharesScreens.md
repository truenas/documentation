...
| **Watch List** | Specifies groups to audit. Click the field to display the dropdown list of group options. Leave blank to include all SMB users with access to the share. Auditing applies only to user accounts that are members of groups in this list. If the same user belongs to groups in both the **Watch List** and **Ignore List**, the watch list takes precedence and operations are audited. |
