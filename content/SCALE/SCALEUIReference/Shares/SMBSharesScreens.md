...
A **warning component** displays on both the Add SMB and Edit SMB screens after selecting the **MacOS Media Share** option in **Purpose** if the **Enable Apple SMB2/3 Protocol Extensions** option is not already enabled on the **Advanced Options** of the [**SMB** service screen]({{< ref "SMBServicesScreen.md" >}}). This warning indicates that the share requires Apple extensions to function properly.
...
| Setting | Description |
|---------|-------------|
| **Use Apple-style Character Encoding** | Automatically enabled and disabled (non-editable) for MacOS Media Share. This implements Apple name mangling for NTFS illegal characters, which is required for proper operation of Apple Media & Entertainment applications. This setting cannot be changed for this share purpose as it is essential for compatibility with Final Cut Pro, Logic Pro, and other professional media tools. |
{{< /truetable >}}
{{< /tab >}}
{{< /tabs >}}
