...
The interactive arguments editor provides a Text User Interface (TUI) where parameters and options can be configured. The TUI also provides some information on required parameters, defaults, and expected input types (string, boolean, integer, or array).
...
| `locked=` | Boolean | If set to `true`, prevents the user from logging in or using password-based services until this parameter is unset. Locking an account is only possible when Disable Password is false and a Password has been created for the account. Defaults to `false`. <br> Ex. `locked=true` <br> &emsp; Where `true` is a boolean variable. | No |
...
| `attributes=` | Object | The **attributes** dictionary is a general-purpose object for storing arbitrary user information. This parameter can be used to store custom metadata and other information relevant to the user. Custom keys and corresponding values can relate to specfic needs and use cases. <br> Ex. `attributes={"favorite_color": "blue"}` <br> &emsp; Where `favorite_color` is a new or existing key and `blue` is a corresponding value. | No |
{{< /truetable >}}
{{< /expand >}}
## Delete Command
...
The `get_next_uid` command does not require entering parameters or values.
...
The `has_local_administrator_set_up` command does not require entering parameters or values.
...
Enter the command with no additional attributes or parameters to perform a basic query of all local users.

Add additional parameters to return the value of the specified key(s) for all users.
There are 23 `query` parameters available.
...
Where *3001* is the UID of the account to update, *favorite_color* is the desired parameter, and *blue* is the corresponding value.
...
Enter the command with no additional parameters to return shell choices available to all users.
...
The `update` command updates the attributes of an existing user. For available parameters, see [`create`](#create-configuration-parameters).
...
Enter all parameters and corresponding values to update.
