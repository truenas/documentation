...
| **Sync** | Sets the data write synchronization. Options are:<ul><li>**Inherit** takes the sync settings from the parent dataset.</li><li>**Standard** uses the settings requested by the client software.</li><li>**Always** waits for data writes to complete.</li><li>**Disabled** never waits for writes to complete.</li></ul> |
