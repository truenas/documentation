...
| **Sparse** | Enables using [thin provisioning](https://docs.truenas.com/references/thin-provisioning/) where disk space for this volume is allocated on-demand as new writes are received. Use caution when enabling, as writes can fail when the pool is low on space. |
