...
The table shows the used and available storage space for that dataset, the encryption status (locked, unlocked, or unencrypted), the role of that dataset, and the service it uses (i.e., the system dataset, a share, virtual machine, or application).
...
The common settings are **Parent Path**, **Name**, and the **Dataset Preset** (previously know as the share type).
