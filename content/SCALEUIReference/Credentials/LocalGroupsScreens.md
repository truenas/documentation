...
| **SMB Group** | Select to make this group available for permissions editors over [SMB protocol]({{< relref "/SCALETutorials/Shares/" >}}) (and the share ACL editor). It is not used for SMB authentication or determining the user session token or internal permissions checks. |
...
Assigned administrator roles display on the [Users Screen]({{< relref "LocalUsersScreensSCALE" >}}).
