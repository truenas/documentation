...
The **Static Routes** widget on the **Network** screen displays static IP addresses (aliases) configured as static routes. Use this to manually enter routes to network destinations outside the TrueNAS network so the router can send packets to a destination network.
