...
* **Cause**: When ZFS has fast special vdev SSDs, sufficient RAM, and is not limited by disk I/O, then hash calculation becomes the next bottleneck.
  ...
  This is often encountered with [pool scrubs]({{< relref "ScrubTasksSCALE.md" >}}) and it can be necessary to pause the scrub temporarily when other tasks are a priority.
