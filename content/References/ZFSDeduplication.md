...
[Special vdev]({{< ref "/SCALE/Storage/Pools/CreatingFusionPools" >}}) SSDs receive continuous, heavy I/O.
...
* **Cause**: When ZFS has fast, special vdev SSD disks, sufficient RAM, and is not limited by disk I/O, then the hash calculation becomes the next bottleneck.
 ...
 This is often encountered with [pool scrubs]({{< ref "/SCALE/Storage/Pools/ManagePools" >}}) and it can be necessary to pause the scrub temporarily when other tasks are a priority.
