...
TrueNAS Mini systems ship with Western Digital NAS and NL-SAS respectively for good reason, and understanding the alternatives will explain this decision.
{{< /tab >}}
{{< tab "SATA NAS Disks" >}}
...
Therefore, capacity-oriented TrueNAS systems ship with [Western Digital UltraStar](https://www.westerndigital.com/products/data-center-platforms) NL-SAS disks thanks to the all-around perfect balance of capacity, reliability, performance, and flexibility that NL-SAS drives offer.
{{< /tab >}}
{{< tab "SAS Disks" >}}
...
For example, Enterprise SAS drives were discontinued from the TrueNAS product lines and replaced almost entirely by flash drives (SSDs or NVMe) in 2016 due to their superior performance/cost ratio.
{{< /tab >}}
...
Flash storage SSDs can be used in place of HDDs for primary storage on a TrueNAS system, resulting in a faster, though either smaller or more expensive storage solution.
...
{{< /tab >}}
{{< tab "USB Hard Disks" >}}
USB-connected hard disks should be avoided for primary storage with TrueNAS but can be used for very basic backups in a pinch.
...
Check the number of hours on all “new” drives using `smartctl(8)` as they could be “recertified”.
...
In addition, there are countless warnings against using hardware RAID cards with TrueNAS, but it’s really not that simple.
...
For ZFS and TrueNAS, this behavior virtually eliminates any chances that RAM errors will be passed to the drives and cause corruption of the ZFS pools or errors in the files.
...
* Samba, the TrueNAS SMB daemon is lightly-threaded, so a higher-frequency CPU with fewer cores will usually perform best for SMB-only workloads.
...
* AMD CPUs are making a comeback thanks to the Ryzen and EPYC (Naples/Rome) lines. Support for these platforms has been relatively limited on FreeBSD and, by extension, TrueNAS CORE. However, there is substantial Linux support and TrueNAS SCALE should work with AMD CPUs without issue.
...
The 80 plus rating indicates how much of the power drawn from the wall is lost as heat, noise, and vibrations, instead of doing useful work like powering your components.
...
Home labs can now realistically employ 40GB and higher networking components, and home users are discovering the same complications with these speeds that have been found by Enterprise customers.
