...
However, this means that to restore any cloud credentials from a TrueNAS configuration file, you must enable **Export Password Secret Seed** when generating that [configuration backup]({{< ref "UsingConfigurationBackups" >}}).
...
See [Cloud Credentials]({{< ref "/uireference/system/cloudcredentials" >}}) for provider-specific fields and settings.
