...
A ZFS Volume (Zvol) is a [dataset]({{< ref "/CORETutorials/Storage/Pools/Datasets" >}}) that represents a block device.
These are needed when configuring an [iSCSI Share]({{< ref "/CORETutorials/Sharing/iSCSI/AddingiSCSIShare" >}}).
...
See [Zvols Screen]({{< ref "/UIReference/Storage/Pools/ZvolsScreen" >}}) for more information on zvol settings.
...
When the selected zvol is cloned from an existing [snapshot]({{< ref "/CORETutorials/Storage/Snapshots" >}}), **Promote Dataset** is available.
...
When the zvol is created with [encryption]({{< ref "/CORETutorials/Storage/Pools/StorageEncryption" >}}) enabled, additional **Encryption Actions** are displayed.
