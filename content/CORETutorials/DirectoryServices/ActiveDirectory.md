...
When the import completes, AD users and groups become available. These have basic dataset permissions or an [Access Control List (ACL)]({{< ref "/CORETutorials/Storage/Pools/Permissions" >}}). Enabled is the default status for the TrueNAS cache.

Joining AD adds default [Kerberos]({{< ref "/CORETutorials/DirectoryServices/Kerberos" >}}) realms and generates a default `AD_MACHINE_ACCOUNT` keytab.
