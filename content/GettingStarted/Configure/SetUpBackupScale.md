...
After installing and completing your SCALE system configuration, [create a boot environment]({{< relref "ManageBootEnvironSCALE" >}}) to use as a restore point.
...
* [Cloud sync tasks]({{< relref "/ScaleTutorials/DataProtection/CloudSyncTasks/" >}})
* [Replication tasks](#using-replication)
* [Rsync tasks]({{< relref "/SCALETutorials/DataProtection/RsyncTasksSCALE" >}})
...
See [Managing TrueCloud Backup Tasks]({{< relref "TrueCloudTasks" >}}) for a full tutorial.

See [Adding Cloud Credentials]({{< relref "/scaletutorials/credentials/backupcredentials/addcloudcredentials" >}}) for information on connecting TrueNAS SCALE to other cloud storage providers.
...
Replication needs an existing [periodic snapshot task]({{< relref "PeriodicSnapshotTasksSCALE" >}})** to run before the replication task runs or the replication task fails.
