...
This is used for [major version upgrades]({{< ref "/CORETutorials/UpdatingTrueNAS/#upgrade-via-iso" >}})
{{< /expand >}}
...
If you're still researching what kind of hardware to use with TrueNAS, read over the very detailed [CORE Hardware Guide]({{< ref "COREHardwareGuide" >}}).
...
See [Pool Creation]({{< ref "/CORETutorials/Storage/Pools/" >}}) for descriptions of the various pool (vdev) types and layouts
...
The [Console Setup Menu]({{< ref "ConsoleSetupMenu" >}}) displays when the system boots successfully.
...
The next step is to [log in to the web interface]({{< ref "LoggingIn" >}}) using the administrative account credentials and begin configuring the system.
