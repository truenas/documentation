...
description: "Provides instructions on using Advanced Replication and lists other tutorials for configuring advanced ZFS snapshot replication tasks in TrueNAS SCALE."
...
6. Specify the source and destination paths. Adding /*name* to the end of the path creates a new dataset in that location.
   ...
   If the file browser shows a connection error after selecting the correct **SSH Connection**, you might need to log in to the remote system and [configure it to allow SSH connections]({{< ref "/SCALEUIReference/Credentials/BackupCredentials/" >}}).
