...
{{< hint type=important >}}
Restic uses the password created for the backup repository to encrypt and manage the repository. Without this password, users cannot access or restore backup data, and restic cannot decrypt the repository.

TrueNAS only stores the password as part of the TrueCloud backup task, and passes the password to restic using the environment variable RESTIC_PASSWORD each time the repository is accessed. This is done so that restic can further manage and encrypt the repository.
{{< /hint >}}
