...
Storage backups must be handled through the [ZFS snapshot]({{<ref "PeriodicSnapshotTasksSCALE" >}}) feature or other [backup options]({{< ref "/SCALETutorials/DataProtection/" >}}).
TrueNAS [applications]({{< ref "/SCALETutorials/Apps/" >}}) also use separate upgrade and container image management methods to provide app update and rollback features.
