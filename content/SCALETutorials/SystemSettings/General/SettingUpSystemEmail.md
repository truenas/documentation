...
{{< hint type=note >}}
[Scrub Task]({{< ref "ScrubTasksSCALE" >}}) issues and [S.M.A.R.T. reports]({{< ref "SMARTTestsSCALE" >}}) are mailed separately to the address configured in those services.
{{< /hint >}}
...
Follow the directions in [Configuring the Admin User Email Address](#configuring-the-admin-user-email-address) above for an existing user or see [Managing Users]({{< ref "ManageLocalUsersSCALE" >}}) for a new user.
