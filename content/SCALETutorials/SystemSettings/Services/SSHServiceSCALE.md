...
TrueNAS shows an error if users activate Global 2FA (Two-Factor Authentication) and attempt to enable password-based SSH authentication without a 2FA token. See [Managing Global 2FA]({{< relref "/scaletutorials/systemsettings/advanced/manageglobal2fascale/" >}}) for more information.
