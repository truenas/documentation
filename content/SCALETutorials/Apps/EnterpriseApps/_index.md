...
Only install applications with the assistance of an iXsystems Support.
...
## File Sharing
{{< hint type=important >}}
Since TrueNAS considers shared host paths non-secure, apps that use shared host paths (such as those services like SMB are using) might fail to deploy.
...
## Using Custom App
The **Custom App** button starts the a configuration wizard where users can install applications not included in the approved application catalog.
...
Determine the container and node port numbers. TrueNAS SCALE requires a node port to be above 9000.
