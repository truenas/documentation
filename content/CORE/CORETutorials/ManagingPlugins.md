...
* Create a [data pool]({{< relref "/CORE/CORETutorials/PoolCreate.md" >}}) if one doesn't exist. A pool must be available for plugin storage.
...
One option to back up this stored data is to create a [local replication]({{< relref "CORE/CORETutorials/LocalReplication.md" >}}).
