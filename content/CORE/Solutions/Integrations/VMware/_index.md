...
The setup of VMware iSCSI to TrueNAS requires that ESXi hosts be set up as initiators and TrueNAS storage arrays are set up as targets.
To improve the automation of VMware and TrueNAS, iXsystems has developed a [vCenter plugin for TrueNAS]({{< relref "CORE/Solutions/Integrations/VMware/TrueNASvCenterPlugin/_index.md" >}}) that automates the creation and assignment of LUNs.

Setup is further simplified by having the vCenter Plugin automate many of the configuration tasks.
The plugin does this by using the TrueNAS REST APIs.
