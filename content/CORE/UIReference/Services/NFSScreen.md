...
| **NFSv3 ownership model for NFSv4** | Select checkbox when NFSv4 ACL support is needed without requiring the client and the server to sync users and groups. |
...
| **Support >16 groups**  | Select checkbox when a user is a member of more than 16 groups. This assumes group membership is configured correctly on the NFS server. |
