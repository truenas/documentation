...
* Prepare TrueNAS for the app installation by [creating the datasets](#creating-the-storj-datasets-on-truenas) for the application storage, and a new user if not using the default apps user.
   ...
   If using ports other than the assigned default ports (**20909** and **28967**), go to [default port list]({{< relref "/solutions/optimizations/security/#truenas-default-ports" >}}) for a list of available or unavailable port numbers to verify the port numbers you want to use are available options.
...
4. Accept the default **Network Configuration** settings.
   If you want to use different ports, check the [default port list]({{< relref "/solutions/optimizations/security/#truenas-default-ports" >}}) for a list of available or unavailable port numbers.
