...
TrueNAS creates an **ix-apps** dataset on the chosen pool and uses it to store all container-related data.
...
Do not change the version number for official apps or those included in a TrueNAS app catalog.
When a new version becomes available, an **Update** banner shows on the **Installed** application screen, the app row shows an update alert, and the **Application Info** widget showss an update button.
...
Refer to individual tutorials in the [Stable]({{< relref "/TruenasApps/StableApps/_index.md" >}}), [Community]({{< relref "/SCALE/SCALETutorials/Apps/CommunityApps/_index.md" >}}), or [Enterprise]({{< relref "/TruenasApps/EnterpriseApps/_index.md" >}}) sections of the Documentation Hub for more details on configuring application settings.
...
{{< trueimage src="/images/Apps/ResourcesConfigurationAllocateGPU.png" alt="Select GPU Allocation" id="Select GPU Allocation" >}}
