...
</file>. Refer to documentation on upgrading existing systems found [here]({{< relref "/TrueNASUpgrades" >}}).

If side-grading (migrating) from an earlier release to the latest TrueNAS release, follow guidance in the [Preparing to Migrate]({{< relref "/SCALE/GettingStarted/Migrate/MigratePrep" >}}) article before beginning the migration.

If clean-installing TrueNAS for the first time, follow guidance in the [Installation Instructions]({{< relref "/SCALE/GettingStarted/Install" >}}) articles.

### Prerequisites
If you are installing TrueNAS on servers other than licensed TrueNAS Enterprise systems, you should read and follow guidance in the [TrueNAS Hardware Guide]({{< relref "SCALEHardwareGuide" >}}).

## Configuring TrueNAS
After installing TrueNAS, users must configure network, storage, users, sharing, and data backup solutions. See [Configuration Instructions]({{< relref "/SCALE/gettingstarted/configure" >}}) for more information.
...
See [Using Administrator Logins]({{< relref "AdminRoles" >}}) for more information on administration user accounts.
...
See [SSH Service]({{< relref "SSHServiceSCALE" >}}) for more information.
...
See [Managing Users]({{< relref "ManageLocalUsersSCALE" >}}) and [Using Administrator Logins]({{< relref "AdminRoles" >}}) for more information on creating users and allowing `sudo` command access.
...
See [Managing Global Two-Factor Authentication]({{< relref "ManageGlobal2faSCALE" >}}) for more information
...
See [Audit Logs]({{< relref "AuditingSCALE" >}}) for more information on TrueNAS audit logs and configuring auditing settings.
...
#### System Logging
Configure TrueNAS to send system logs to an external server using the **Syslog** widget on the **System > Advanced Settings** screen in [release 24.10 and later]({{< relref "managesyslogsscale" >}}).
...
To increase network, consider configuring allowed domains on the [**Network > Global Configuration** screen]({{< relref "ManagingGlobalConfig" >}}).

To increase share security, configure allowed host names or IP addresses for [SMB shares]({{< relref "/scale/scaletutorials/shares/smb/_index/#configuring-share-advanced-options-settings" >}}) or [NFS shares]({{< relref "/SCALE/SCALETutorials/Shares/AddingNFSShares/#adding-nfs-share-networks-and-hosts" >}}).
...
See [Setting Up Permissions]({{< relref "PermissionsSCALE" >}}) for more information.
...
See [Windows Shares]({{< relref "/SCALE/SCALETutorials/Shares/SMB" >}}) for more information on SMB shares, creating shares, and configuring settings including permissions.

NFS shares do not have the same setting options and rely on the dataset ACL settings.
See [NFS Shares]({{< relref "AddingNFSShares" >}}) for more information.
...
See [Storage Encryption]({{< relref "EncryptionSCALE" >}}) for more information on encrypting datasets.
...
See [Encrypting Cloud Sync Tasks]({{< relref "/SCALE/SCALETutorials/dataprotection/CloudSyncTasks" >}}) for more information on encrypting cloud sync tasks, or [Adding Transfer Encryption]({{< relref "/scale/scaletutorials/dataprotection/replication/advancedreplication" >}}) for information on adding encryption to remote replication tasks.
...
See [Boot Pool Management]({{< relref "ManageBootEnvironSCALE" >}}) for more information on working with boot pool environments.
