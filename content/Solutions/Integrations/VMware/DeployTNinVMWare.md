...
* Visit the [TrueNAS CORE Hardware Guide]({{< relref "/content/core/gettingstarted/corehardwareguide.md" >}}) and take note of the minimal system requirements.
  ...
  This provides guidance on how much memory, the number of CPUs, and drive size you need to configure. For example, you need a minumum of 2 CPUS, 32 GB memory, and two drives each with at least 16 GB storage but you can increase if you want to improve performance. 
...
* Determine your data storage requirements. Consider the number of storage pools and the type of storage you need for your deployment or how you plan to use the TrueNAS. 
  ...
  For example, if you want a mirror layout you need a minimum of three drives. One for the boot drive and two for the mirror.
  If you want a mirror with a hot spare, you need a minimum of four drives. One for the boot drive, two for the mirror and one for the hot spare.
...
* On the **Customize settings** wizard screen set **CPU** to **2**, set **Memory** to **32 GB**, and **Hard disk 1** to **16 GB**.
...
Click **Power on** and then click **Console** to display the dropdown list of console options. When the console opens it should display the **TrueNAS 13.0-RELEASE Console Setup** screen.
...
![EditSettingsForTruenas1](/images/VMWareESXi/EditSettingsForTruenas1.png "Truenas1 VM Details")
